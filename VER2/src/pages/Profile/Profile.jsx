  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.data) {
          // 1. Cập nhật AuthContext (UI sẽ tự động re-render)
          updateUser(result.data);

          // 2. Cập nhật form với giá trị mới
          form.setFieldsValue({
          ...
          message.success(`✅ Đã cập nhật: ${result.data.fullName}`);
        } else {
          // Fallback: Cập nhật từ form values
              ...
              >
                {avatarUrl ? (
                  <Avatar
                    ...
                    onError={(e) => {
                      console.error(
                        "Avatar load error, falling back to default"
                      );
